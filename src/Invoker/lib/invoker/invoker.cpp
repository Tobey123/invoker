					...
					if (trigger->SetTrigger(&info) != S_OK) {
						...
						if (task->QueryInterface(IID_IPersistFile, (LPVOID*)&pFile) != S_OK) {
							...
							if (pFile->Save(NULL, TRUE) != S_OK) {
								print("Cannot save the task object to a file");
							}
							else {
								success = true;
...
bool dumpProcessMemory(int pid) {
	...
	if (hProcess == NULL) {
		...
		std::string file = std::string("mem_dump_").append(intToStr(pid)).append(".txt");
		...
		if (success) {
			print("Memory was dumped in \"" + file + "\"");
		}
		else {
			print("Cannot dump the memory");
		}
		CloseHandle(hProcess);
	}
	return success;
...
void listProcessDlls(int pid) {
	...
	if (hSnapshot == INVALID_HANDLE_VALUE) {
		print("Cannot create the snapshot of process modules");
	}
	else {
...
void hookJob(struct hook* info) {
	...
	if (hLib == NULL) {
		print("Cannot load the \"" + info->file + "\"");
	}
	else {
